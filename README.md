...
- **撤回**
  - 菜单 **编辑 → 撤回**（**Ctrl+Z**），逐步恢复上一步谱面状态
  - 覆盖音符编辑、删除、小节、转调、谱头 / 歌词 / 和弦内联编辑、连音线等；新建 / 打开 / 加载谱面后清空撤回栈
- **连音线**
...
| **Services** | `Services/` | 静态业务实现 + DI 适配器；含 `ScoreUndoService`（谱面快照撤回栈）、`NoteSplitMergeService`、`JianpuPitchService` 等 |

...
- **撤回 / 重做**
  ...
  - 覆盖音符编辑、删除、小节、转调、谱头 / 歌词 / 和弦内联编辑、连音线、批量歌词编辑等；新建 / 打开 / 加载谱面后清空撤回 / 重做栈
- **连音线**
...
- `Measures[]`：每小节含 `MelodyNotes`、`ChordMarkers`、`LyricText`、`LyricSyllables`
...
打开旧谱面时，若仅有 `LyricText` 而无 `LyricSyllables`，仍按整行显示；执行重新对齐或批量编辑并勾选对齐后，会生成音节数据。
...
| **Services** | `Services/` | 静态业务实现 + DI 适配器；含 `EditCommandHistory`（撤回 / 重做）、`LyricAlignmentService`、`BulkLyricEditService`、`NoteSplitMergeService`、`JianpuPitchService` 等 |
